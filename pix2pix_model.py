...
def define_discriminator(image_shape):
	...
	#the sigmoid activation function will be applied independently to each element of this tensor.
...
def generate_real_samples(dataset, n_samples, patch_shape):
	...
	#print("ix", ix, "trainA.shape[0]" , trainA.shape)
...
def generate_fake_samples(g_model, samples, patch_shape):
	...
	#print("IN gernerate fake samples", "len(x)", len(X))
...
def train(d_model, g_model, gan_model, dataset, n_epochs=100, n_batch=1):
	...
	# print("d_model.output_shape",d_model.summary())
	...
	for i in tqdm(range(n_steps)):
		...
		#The discriminator's output shape is (1, 16, 16, 1), where each element represents the 
		#probability that the corresponding patch in the input images is real (as opposed to fake).
